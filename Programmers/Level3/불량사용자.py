...
"""
알고리즘
1. user_id 배열에 대해 banned_id 개수만큼 순열 생성
2. 1의 순열과 banned_id 배열이 일치하는지 확인
2-1 문자열 길이 같은지 체크
2-2. 각 문자에 대해 일치하는지 확인
"""
...
def str_check(banned_id, data):
    # 2-2
    for i in range(len(banned_id)):
        if banned_id[i] == "*":
            continue
        elif banned_id[i] != data[i]:
            return False
    return True


def is_same(banned_id, data):
    for i in range(len(banned_id)):
        # 2-1
        if len(banned_id[i]) != len(data[i]):
            return False
        elif str_check(banned_id[i], data[i]) is False:
            return False
    return True
