...
def find(parents, money, n, answer):
    if parents[n] == n or money // 10 == 0:
        answer[n] += money
        return
    commission = money // 10
    profit = money - commission
    answer[n] += profit
    find(parents, commission, parents[n], answer)
...
def solution(enroll, referral, seller, amount):
    ...
    for i in range(n):
        if referral[i] == "-":
            ...
            parents[i+1] = tree[referral[i]]

    for i in range(len(seller)):
        find(parents, amount[i] * 100, tree[seller[i]], answer)
    return answer[1:]
